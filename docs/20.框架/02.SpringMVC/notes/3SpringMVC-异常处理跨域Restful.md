...
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-ce2ba12b22274cb79e648053091f5cab.png)

![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-81ed407d5d9042f8a23df2b59056f5fd.png)
...
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-96907f4334a043edb06e83024c775e0e.png)
### 2.2 跨域环境搭建
...
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-db9fde58456047cba14a797e48aa07bd.png)
### 2.3 跨域访问支持  
...
* 请求处理过程解析  
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-1035cc292eca45b59947ff5af1dc47d0.png)
...
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-65fa8f961196447fa24c8deea7bf4e59.png)


拦截器VS过滤器
   归属不同： Filter属于Servlet技术， Interceptor属于SpringMVC技术
   拦截内容不同： Filter对所有访问进行过滤， Interceptor仅针对SpringMVC的访问进行增强  
  ![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-cb9d0a9142324e82a247a3285c93bf62.png)
### 3.2 自定义拦截器开发过程
...
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-d3d6e1d469f24dcc9f9c917734f292d7.png)
...
* 参数
   request:请求对象
   response:响应对象
   handler: 被调用的处理器对象，本质上是一个方法对象，对**反射中的Method对象**进行了再包装
* 返回值
...
 参数
 modelAndView:如果处理器执行完成具有返回结果，可以读取到对应数据与页面信息，并进行调整  (对页面信息的修改)
...
```
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-7375ced273074c11b2ab4f1ccd67e91e.png)


### 3.6 多拦截器配置
只要前置方法执行了，后置方法一定执行；
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-bf54d1c9a69d44dbb69268af1fefaf13.png)
**责任链模式**
-  责任链模式是一种行为模式
-  特征：
...
* 使用注解实现异常分类管理
   名称： @ControllerAdvice 是对Controller增强的一个通知
   类型： 类注解
   位置：异常处理器类上方
   作用：设置当前类为异常处理器类
   范例：
...
* 使用注解实现异常分类管理
 ...
 ```java
@ExceptionHandler(Exception.class) //匹配那个异常类
@ResponseBody
public String doOtherException(Exception ex){
    return "出错啦，请联系管理员！ ";
}  

@ExceptionHandler(XXXXException.class) //匹配那个异常类
@ResponseBody
public String doOtherException(Exception ex){
    return "XXXX出错啦，请联系管理员！ ";
}  
 ```
- 注解和定义接口的两个的区别：
注解的是在加载请求之前就会加载，但是定义接口的是在，接收到请求之后才会生效的； 一般来说还是使用注解开发的样式；

### 4.3 异常处理解决方案
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-1627b78ef3cf47d68305e4dba73a8a6b.png)
* 异常处理方案
  * 业务异常：
     发送对应消息传递给用户，提醒规范操作
  * 系统异常：
...
* 上传文件过程分析  
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-7cead97f3e874c53bc8bdd7eb2c5f2e0.png)
...
* 文件上传下载实现
  ...
  * 控制器  
    ...
    ```
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-794d695f5edc44f5a7b91aec550c942c.png)
### 5.2 文件上传注意事项
...
public String fileupload(MultipartFile file,MultipartFile file1,MultipartFile file2, HttpServletRequest request) throws IOException {
    ...
    //        System.out.println(file.isEmpty()); 是不是空文件
...
#### 5.4.3 Restful开发入门  
restful 一般就是展示数据的，一般用@RestController直接返回数据；
```java
//设置rest风格的控制器
@RestController
//设置公共访问路径，配合下方访问路径使用
...
```
get 查询 delete 删  put 添加 post 改
- 新增商品：put:product/manage/{productId}
- 修改商品：post:product/manage/{productId}
- 删除商品：delete:product/manage/{productId}
- 查询商品：get:product/manage/{productId}
![image.png](https://cdn.jsdelivr.net/gh/Control-body/tuChuang/2022/04/image-7cb8d62c6cd540af8422207c2d9ff286.png)
### 5.5 postman工具安装与使用
